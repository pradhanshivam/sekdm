    ...
    def loadModel(self):
        ...
        if model_files:
            ...
            for file in model_files:
                try:
                    model = KD_EGNN(
                        infeature_size=args.infeature_size,
                        outfeature_size=args.outfeature_size,
                        nhidden_eg=args.nhidden_eg,
                        edge_feature=args.edge_feature,
                        n_eglayer=args.n_eglayer,
                        nclass=args.nclass,
                        device=args.device
                    )
                    state_dict = torch.load(file, map_location=args.device)
                    model.load_state_dict(state_dict, strict=False)
                    model.eval()
                    self.model_list.append(model)
                except Exception as e:
                    QMessageBox.critical(self, 'Error', f'Load model failed: {str(e)}', QMessageBox.Ok)
                    return False
            self.logTextEdit.append('Load model successfully.')
            self.logTextEdit.append(f'Model number: {len(model_files)}')
            return True
        return False
